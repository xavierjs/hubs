function ThreeMorphAnimGeomBuilder(specArg){
  ...
  // Construct faces:
  const facesIndices = [],
    uviByVertexIndex = new Int32Array( vertices.length ),//lib_array.create(-1, vertices.length),   
    verticesDuplicatedIndices = [];

  for (let i=0; i<uviByVertexIndex.length; ++i){
    uviByVertexIndex[i] = -1;
  }   
  ...
  spec.data['base']['faces'].forEach(function(f){
    let via = f[0][0];
    let vib = f[1][0];
    let vic = f[2][0];
  ...
  // Construct morphs:
  console.log('INFO in ThreeMorphAnimMeshBuilder: ', spec.data['morphs'].length, 'morphs found');

  const morphs = [];
  const nVertices = nVertices0 + verticesDuplicatedIndices.length;
  spec.data['morphs'].forEach(function(morph, morphIndex){
...
export default ThreeMorphAnimGeomBuilder;